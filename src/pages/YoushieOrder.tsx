...
export default function YoushieOrder() {
  ...
  return <div className="youshie-page order-page">
    ...
    <main className="order-main">
      {searchParams.get('payment') === 'success' && <section className="checkout-status success" style={{ padding: '32px 22px', textAlign: 'center', borderRadius: '24px', background: '#253126', color: '#fff', border: '2px solid #e8c9a0', boxShadow: '0 16px 36px rgba(25,43,29,.22)' }}><div style={{ display: 'flex', justifyContent: 'center', gap: 12, color: '#e8c9a0', marginBottom: 10 }}><Sparkles size={25}/><Check size={26}/><Sparkles size={25}/></div><strong style={{ display: 'block', fontSize: '26px', lineHeight: 1.15 }}>Hooray! Your Youshie is on its way to being real.</strong><span style={{ display: 'block', marginTop: 10, lineHeight: 1.6 }}>KiwiKoru is doing a happy little dance. {confirmation === 'sent' ? 'Your KiwiKoru confirmation is on its way by email.' : confirmation === 'sending' ? 'We’re preparing your KiwiKoru confirmation email…' : confirmation === 'error' ? 'Your payment is safe and we will contact you shortly.' : 'We’re preparing your KiwiKoru confirmation email.'}</span></section>}
