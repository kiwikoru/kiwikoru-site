    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6">
      ...
      <div className="grid lg:grid-cols-[1fr_380px] gap-8 items-start">
        <form id="print-order-form" onSubmit={checkout} className="bg-white rounded-3xl border border-gray-200 p-6 md:p-9 shadow-sm">
          ...
          {destination !== 'australia' && <label className="mt-3 flex items-center gap-3 rounded-xl bg-off-white p-3 text-sm text-forest-dark"><input type="checkbox" className="accent-forest" checked={rural} onChange={e => setRural(e.target.checked)}/><span className="flex-1">Rural delivery</span><strong>+ NZ$6.00</strong></label>}
        ...
        <aside className="bg-forest-dark text-white rounded-3xl p-7 shadow-xl lg:sticky lg:top-24"><div className="flex items-center gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-xl bg-white/10"><FileBox className="text-gold" size={22}/></span><div><small className="text-gold-light font-bold tracking-wider">YOUR ORDER</small><h2 className="text-2xl font-semibold">Order summary</h2></div></div><div className="mt-7 space-y-4 text-sm"><div className="flex justify-between gap-4"><span className="text-white/65">3D print</span><strong>NZ${(quote?.price || 0).toFixed(2)}</strong></div><div className="flex justify-between gap-4"><span className="text-white/65">Delivery</span><strong>NZ${shipping.toFixed(2)}</strong></div><div className="flex justify-between border-t border-white/15 pt-5 text-xl"><span>Total</span><strong className="text-gold-light">NZ${total.toFixed(2)}</strong></div></div><button type="submit" form="print-order-form" disabled={!quote || !order?.modelFile || working} className="mt-6 w-full rounded-xl bg-gold py-3.5 font-bold text-forest-dark shadow-md transition hover:bg-gold-light hover:-translate-y-0.5 disabled:opacity-40">{working ? 'Opening checkout…' : 'Proceed to secure payment'}</button><div className="mt-7 space-y-4 text-xs text-white/65"><p className="flex gap-3"><Truck size={18} className="shrink-0 text-gold-light"/>Delivery pricing: NZ North or South Island NZ$15, Australia NZ$30, rural NZ surcharge NZ$6.</p><p className="flex gap-3"><Mail size={18} className="shrink-0 text-gold-light"/>You’ll receive confirmation after payment and another email when your order is ready to dispatch.</p><p className="flex gap-3"><MapPin size={18} className="shrink-0 text-gold-light"/>Your address and contact details travel securely with the paid order.</p></div><p className="mt-6 border-t border-white/10 pt-4 text-center text-[11px] text-white/45"><ShieldCheck size={13} className="inline mr-1"/>Payment protected by Stripe</p></aside>
      </div>
    </div>
  </main>
}
